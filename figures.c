...
int     check_if_figure_is_tetramino(char *our_figure)//22lines
{
    int i;
    char *changed_example;
    int x_offset;
...
int     find_quantity_of_figures(char *str, size_t len)//23lines//нужная функция переместить в мэин или еще куда
{
    ...
    while(*str)
    {
     ...
     if(quan * 4 != size_of_sharps || quan * 12 != size_of_dots || (size_of_slashn != (quan * 5 - 1)))
         return (-1);
    return (quan);
}
...
int    check_figures(int quan_of_figures, char *arr)//15lines//оставляем тут
{
    char *test_arr;
    while (quan_of_figures >= 0)
    {
...
int     check_validity(char *av)//9lines//оставляем тут
{
    int quan_of_figures;
    int right_figures;
    quan_of_figures = find_quantity_of_figures(av, ft_strlen(av));
    printf("quantity of figures is : %i\n", quan_of_figures);
    if (quan_of_figures)
        right_figures =  check_figures(quan_of_figures, av);
    printf("right_figures is: %i\n", right_figures);
    if (right_figures > 0 && quan_of_figures > 0 && quan_of_figures <= 26)
        return (1);
    //free(golova spiska)
    return (-1);
}